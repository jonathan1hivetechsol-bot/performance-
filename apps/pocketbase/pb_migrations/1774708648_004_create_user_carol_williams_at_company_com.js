...
migrate((app) => {
  ...
  record.set("email", "carol.williams@company.com");
  record.setPassword("Employee123!");
  record.set("name", "Carol Williams");
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "carol.williams@company.com");
    app.delete(record);
  } catch (e) {
